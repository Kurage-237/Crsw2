...
class JSONVacancyFileHandler(VacancyFileHandler):
    ...
    def write_vacs(self, data: List[Dict[str, Any]], **kwargs: Any) -> None:
        try:
            existing = self.load_vacs()
        except FileNotFoundError:
            existing = []

        existing_urls = utils.extract_existing_urls(existing)
        new_data = utils.filter_new_vacancies(data, existing_urls)

        with open(self.__filepath, "w", encoding="utf-8") as f:
            json.dump(existing + new_data, f, ensure_ascii=False, **kwargs)
